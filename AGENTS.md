...
- **Email login is the default** on `/login`; ATProto handle login is a
  collapsible alternative. Set `EPDS_URL=https://certified.one` locally and
  in production. `/oauth/login` authorizes the configured ePDS service URL,
  then adds `login_hint` and `prompt=login` to the redirect, as in simocracy-v2.
  Both methods share the SDK callback and DID session. Do not log email hints
  or authorization URLs. The provider may auto-send an OTP when its page's
  JavaScript runs; smoke-test redirect/HTML without sending unsolicited codes.
